...
def delete_purchases(id):
    delete(f'http://1e8579f3.ngrok.io/api/purchases/{id}').json()
...
def register(update, context):
    ...
    response = get('http://1e8579f3.ngrok.io/api/users').json()
    found = None
...
def things(update, context):
    if update.message.text == 'Я в магазине!':
        ...
        resp = get('http://1e8579f3.ngrok.io/api/purchases').json()
        names = []
